...
Normalizing a quaternion is identical to normalizing a vector. The quaternion $q$ is divided by the norm of the quaternion $\left\lVert q \right\rVert$. When a quaternion is normalized, it is denoted with a circumflex accent ($\hat{q}$):
...
Applying the notation as declared previously, ${^w_b}q$ constitutes the orientation of body frame with respect to world frame. From equation \ref{eq:quaternion_representation}, the subsequent list of properties/operations is derived:

Every quaternion operation ought to follow the succeeding list of properties:
...
The quaternions multiplication, denoted by , is defined by the Hamilton
product as in equation 2.8
...
The quaternion conjugate describes the inverse rotation and is defined as equation 2.9.

\begin{equation}
    \textrm{$_{b}^{w}q$}^* =\textrm{$_{w}^{b}q$} = \left[q_1 - q_2 - q_3 - q_4\right]
\end{equation}

Let’s define the following quaternion representation of the same vector but in each referential by using is pure quaternion as wv = [0 wx wy wz] and bv = [0 bx by b z]. The rotation of vector v from one frame to other, using a quaternion, is performed by equation 2.10.

\begin{equation}
    \textrm{$^{b}v$} = \textrm{$_{b}^{w}\hat{q}$} \otimes \textrm{$^{w}v$} \otimes \textrm{$_{b}^{w}\hat{q}$}^*
\end{equation}

The composition of rotations can be described in quaternions as the product
between quaternions. For example the sequence a b qˆ → b c qˆ is equal to a c qˆ and is defined as equation 2.11.
...
As stated previously, to eliminate the singularities at ±90◦ on the pitch and roll axes, and to keep the computational efficiency optimal, quaternions are used for state attitude representation. To understand quaternions, one must first understand the complex number
